...
if (isset($_POST["mode"]) && $_POST["mode"] === "delete") {
    ...
    // insert into image_board values(null, 'admin', '관리자', '제목', '장소', '기간', '시간', '가격', '등록일', 1, '파일네임', '파일타입', '파일카피');
    $sql = "insert into image_board (id, name, subject, place, period, time, price, regist_day, hit,  file_name, file_type, file_copied) ";
    $sql .= "values(:userid, :name, :subject, :place, :period, :time, :price, :regist_day, 0, ";
    ...
    if (empty($_POST["ripple_content"])) {
        print "<script>alert('내용입력요망!');history.go(-1);</script>";
        exit;
    }
    ...
    if (!$rowcount) {
        print "<script>alert('없는 아이디!!');history.go(-1);</script>";
        exit;
    } else {
        ...
        print "<script>location.href='./image_board_view.php?num=$parent&page=$page&hit=$hit';</script>";
    } //end of if rowcount
