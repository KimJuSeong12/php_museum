...
$userpoint = (isset($_POST["userpoint"])) ? $_POST["userpoint"] : '';
$regist_day = date('Y-m-d H:i:s');

if ($id == "" || $pass == "" || $name == "" || $email == "") {
  die("
  <script>
  alert('데이터 문제 발생');
  </script>");
...
$sql = "insert into members values(null, :id, :pass, :name, :email, :zipcode, :addr1, :addr2, :regist_day, :userlevel, :userpoint)";
...
$stmt->bindParam(':userpoint', $userpoint);
$result = $stmt->execute();
